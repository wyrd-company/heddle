...
case "${collection_directory}" in
    /|"${repository}"|"${repository}/features"|"${repository}/features/heddle")
...
git -C "${repository}" archive "${accepted_head}" -- features/heddle \
    | tar -C "${staged_feature}" --strip-components=2 -xf -
