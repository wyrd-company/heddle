...
describe.skipIf(!t3Binary || !nativeDrivers)(
  ...
  () => {
    it("runs a real Claude Code session that calls a Heddle MCP tool", async () => {
      ...
      await writeFile(
        join(fixture.blueprintsRepositoryRoot, "handoff-templates", "standard.md"),
      ...
      const advanced = await (async () => {
        ...
        throw new Error(
          `Native session never advanced the stage. T3: ${isolated
            .serverLog()
            .slice(-1500)}\nService: ${serviceOutput.slice(-2000)}`,
        );
      })();
      ...
      expect(thread?.modelSelection?.instanceId).toBe(
        QUALIFICATION_EXECUTION.instanceId,
      );
      expect(thread?.modelSelection?.model).toBe(providerAliases.execution.model);
